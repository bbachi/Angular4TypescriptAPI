...
protected  isLoggedInUserInBMFSessionNotNull(req:any):boolean{
	...
	public getLoggedInUserFaxnumber(req:any):string{
		...
		return "";*/
    return req.body.businessInfo.faxNumber;
	}
	...
	public getLoggedInUserPhonenumber(req:any):string{
		...
		return "";*/
      return req.body.businessInfo.phoneNumber;
	}
...
//this is the MASTER NODE RELATIONSHIP ID FOR START/STOP TXN'S
    ...
    //THIS IS FOR GETTING THE PROPERTY NODE GUID TO GET THE BMF ID FOR STRAT/STOP TXN.
	 public  getNONCAAPropertyNodeGuidList(req:any):Array<string>{
  		let nonCAANodeGuidList = new Array<string>();
  		//nonCAANodeGuidList.push(CommonUtil.getBMFSSSession(req).selectedProperty.strNodeGuidForStartStopTxn);
  ...
  private  populateAddSSFailTransactionRequest( errorCode:string, errorMessage:string, ex:any, req:any):AddSSFailTransactionRequest{
   ...
   return request;
    ...
    public getUnavailableDates( esiID:string):string{
 		LoggerUtil.info("esiID value in getUnavailableDates() is====" +esiID);
 		let tdsp:string = this.getTdspForEsiid(esiID);
 		LoggerUtil.info("tdsp value in getUnavailableDates() is====" +tdsp);
    ...
    //get TDSP for ESIID
	public getTdspForEsiid(inputEsiid:string):string{
		LoggerUtil.info("Start CommonUtilityHelper:getTdspForEsiid().==");
		let tdsp:string = "";
		LoggerUtil.info("esiID value in getTdspForEsiid() is====" + inputEsiid);
		let reqst = new TdspByESIDRequest();
		reqst.companyCode = cst.GMESS_CC_0270 ;
        ...
        var p = new Promise((resolve, reject) => {
		  this.myAccountService.getTDSPFromESID(reqst).then(t => {
            ...
            if (null != t) {
                ...
                LoggerUtil.error("TdspFromESIDWSResponseTdspFromESIDResponse is null in CommonUtilityHelper:getTdspForEsiid()...");
		...
		LoggerUtil.info("TDSP in CommonUtilityHelper:getTdspForEsiid().."+tdsp);
	...
	public  getUnavailableDatesForEsiid(inputEsiid:string):string{
		let tdsp:string = this.getTdspForEsiid(inputEsiid);
		let request_date:string = this.getRequestDateStr();
		let end_date:string = this.getEndDateStr();
		return this.getUnavailableDatesForTdsp(tdsp, request_date,
				end_date);
    }
    ...
    private  getEndDateStr():string{
            ...
            Date.prototype.getTime()+62;//add days
            if((Date.prototype.getMonth()+1)<10) {
                endMonth="0"+(Date.prototype.getMonth()+1);
            } else {
                endMonth=""+(Date.prototype.getMonth()+1);
            }

            if((Date.prototype.getDate()) <10) {
                endDay="0"+Date.prototype.getDate();
            } else {
                endDay=""+Date.prototype.getDate();
            }

            let end_date:string = (endMonth)+('/')+(endDay)+('/')+(Date.prototype.getUTCFullYear());
	...
	private getUnavailableDatesForTdsp( tdsp:string,  request_date:string,  end_date:string):string{
        ...
        var p = new Promise((resolve, reject) => {
            this.myAccountService.getTdspSpecificCalendarDates(req).then(t => {
              resolve(t);
              if (null != t) {
                LoggerUtil.info("############################# unavailableDates and TDSP : "+t+"|"+tdsp);
                unavailableDates = t;
              } else{
                  LoggerUtil.error("TdspCalendarDatesWSResponse is null in CommonUtilityHelper:getUnavailableDatesForTdsp()...");
 ...
 public getInvoicesAccountTypeForBilling(premiseType:string, req:any):string{

		 let premiseName = "";
		 if(CommonUtil.equalsIgnoreCase(req.body.propertyType, cst.CUSTOMERTYPE_PM)){
		   ...
		   }else if (CommonUtil.equalsIgnoreCase(req.body.propertyType, cst.CUSTOMERTYPE_HB)){
