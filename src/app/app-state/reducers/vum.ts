...
export function reducer(state= initialState, action: vum.Actions): State {
    ...
    switch(action.type) {
        ...
        case vum.GET_VACANT_DAILY_REPORT_SUCCESS: return {...state,isLoading: true,isLoadingSuccess: false,dailyReportList: action.payload};
        case vum.GET_VACANT_DAILY_REPORT_FAILURE: return {...state,isLoading: true,isLoadingSuccess: false,dailyReportList: action.payload};
