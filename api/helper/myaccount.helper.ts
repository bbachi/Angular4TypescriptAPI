...
import { BMFSSSession, Property, CustomerDetails} from './../model/bmfss.session';
...
export class MyAccountHelper {
      ...
      public getDashBoardData(req: any): Promise<any> {
            ...
            let hierDtlReq = this.populateBpHierarchyDtlsRequest(selectedPropAndNodeGuidList[1],req);
            let selectedProperty = new Property()
            var p = new Promise((resolve, reject) => {
                LoggerUtil.info("GETTING HIERARCHY DETAILS FOR THE NODEGUID LIST:::::"+selectedPropAndNodeGuidList[1])
                this.preloginService.getBPHierarchyDtls(hierDtlReq).then(s => {
                    if(undefined != s && s.dataAvilableForInput){
                        ...
                        this.preloginService.getAccessPrivForProperty(accPrivReq).then(accRes => {
                            if(undefined != accRes && accRes.dataAvilableForInput){
                                LoggerUtil.info("FOUND ACCESS PRIVILEGES FOR THE PROPERTY:::::")
                                ...
                                LoggerUtil.info("GETTING CONTRACT DETAILS FOR THE PROPERTY:::::")
                                this.preloginService.getCustomerContractDetails(contractReq).then(conRes => {
                                    ...
                                    req.session.bmfSSSession.selectedProperty = selectedProperty;
      ...
      private returnSelectedPropertyFromSession(relationshipId:string, req:any): any{
          ...
          let bpDtlsList = req.session.bmfSSSession.bpDtlsList;
          let selectedProperty = new Property();
