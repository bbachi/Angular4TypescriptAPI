             ...
             if(null !=t && t.transactionsFound){
               ...
               if(_.isEqual(txnTypeStr, cst.TXN_TYPE_LOGIN)){
                ...
                let esiidListArr = new Array<Unit>();
                   if(_.isEqual((CommonUtil.getBMFSSSession(req).selectedProperty.propertyType), cst.CUSTOMERTYPE_PM)){
                  		...
                  		t.txnDtls.forEach((tranDtl: any) => {
                  			let esiid = new Unit();
                  			esiid.esiid=(tranDtl.esiId);
                  			esiid.priorityDateSelected=(_.isEqual(tranDtl.priority, "Y"));
                  			esiid.selectedStartDate=(moment(tranDtl.removeStopDate).format("MM/dd/yyyy"));
                  			esiid.strCity=(tranDtl.city);
                  			esiid.strMeterType=(tranDtl.meterType);
                  			esiid.strState=(tranDtl.state);
                  			esiid.strStreetName=(tranDtl.streetName);
                  			esiid.strStreetNum=(tranDtl.streetNumber);
                  			esiid.strUnitNumber=((CommonUtil.isNotBlank(tranDtl.unitNumber) && _.isEqual(tranDtl.unitNumber,("null"))?cst.NA:tranDtl.unitNumber));
                  			esiid.strZipcode=(tranDtl.zipcode);
                  			esiidListArr.push(esiid);
                  		});
                  		txnDtls.esiidList =esiidListArr;
